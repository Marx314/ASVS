...
| **12.3.3** | Vérifier que les métadonnées de nom de fichier soumises par l'utilisateur sont validées ou ignorées pour empêcher la divulgation ou l'exécution de fichiers distants  via des attaques de type RFI (Remote File Inclusion) ou SSRF (Server-Side Request Forgery).  | ✓ | ✓ | ✓ | [98](https://cwe.mitre.org/data/definitions/98.html) |
